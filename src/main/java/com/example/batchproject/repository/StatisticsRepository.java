...
import com.example.batchproject.entity.statistics.AggregatedStatistics;
import com.example.batchproject.entity.statistics.StatisticsEntity;
...
public interface StatisticsRepository extends JpaRepository<StatisticsEntity,Integer> {
    @Query(value = "SELECT new com.example.batchproject.entity.statistics.AggregatedStatistics(s.statisticsAt,SUM(s.allCount),SUM(s.attendedCount),SUM(s.cancelledCount))" +
